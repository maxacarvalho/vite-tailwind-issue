...
export const useFieldsStore = defineStore({
  ...
  actions: {
    async hydrate() {
      const fieldsResponse = await api.get(`/fields`, { params: { limit: 99999 } });
    ...
    async createField(collectionKey, newField) {
    ...
    async updateField(collectionKey, fieldKey, updates) {
      const stateClone = [...this.fields];
    ...
    async updateFields(collectionKey, updates) {
      ...
      try {
        ...
        if (currentUpdate === updateID) {
          this.fields = this.fields.map((field) => {
            if (field.collection === collectionKey) {
              const newDataForField = response.data.data.find((update) => update.field === field.field);
    ...
    async deleteField(collectionKey, fieldKey) {
      const stateClone = [...this.fields];
    ...
    getFieldsForCollectionAlphabetical(collection) {
      return this.getFieldsForCollection(collection).sort((a, b) => {
     ...
     * dot notation.
     */
    getRelationalField(collection, fields) {
      ...
      const relation = relationsStore
        ...
        ?.find(
          (relation) => relation.field === parts[0] || relation.meta?.one_field === parts[0],
        );
