...
export const useRelationsStore = defineStore({
  ...
  actions: {
    async hydrate() {
      const response = await api.get('relations', { params: { limit: 999999 } });
    ...
    getRelationsForField(collection, field) {
      ...
      const relations = this.getRelationsForCollection(collection).filter((relation) => {
        return (
     ...
     */
    getRelationForField(collection, field) {
      const fieldsStore = useFieldsStore();
      const fieldInfo = fieldsStore.getField(collection, field);
