...
function initLocalStore(collection, field, type) {
      ...
      state.relations = [
        {
          ...
          field: '',
          related_collection: 'system_files',
          meta: {
            sort_field: null,
          },
        },
  ...
  function useO2M() {
    ...
    const syncNewCollectionsO2M = throttle(([collectionName, fieldName, sortField]) => {
      state.newCollections = state.newCollections.filter((col) => ['related'].includes(col.$type) === false);
     ...
     */
    function syncOnDeleteTrigger(collection) {
  ...
  function useM2M() {
    ...
    const syncNewCollectionsM2M = throttle(
      ([junctionCollection, manyCurrent, manyRelated, relatedCollection, sortField]) => {
        state.newCollections = state.newCollections.filter(
          (col) => ['junction', 'related'].includes(col.$type) === false,
        );
    ...
    if (type === 'files') {
      nextTick(() => {
        state.relations[1].related_collection = state.relations[1].related_collection || 'system_files';
      });
    }
    ...
    function autoFillFields(relatedCollection) {
     ...
     */
    function getAutomaticJunctionCollectionName(collectionA, collectionB) {
      let index = 0;
      ...
      ([junctionCollection, manyCurrent, manyRelated, oneCollectionField, sortField]) => {
        ...
        if (fieldExists(junctionCollection, oneCollectionField) === false) {
          state.newFields.push({
            ...
            type: 'string', // system_collections.collection is a string
            schema: {},
            meta: {
              hidden: true,
            },
  ...
  function fieldExists(collection, field) {
