  ...
  setup(props) {
    ...
    function useRelation() {
      ...
      const fields = computed(() => {
        ...
        return fieldsStore.getFieldsForCollectionAlphabetical(state.relations[0].collection).map((field) => ({
          text: field.field,
          value: field.field,
          disabled:
      ...
      const hasCorresponding = computed({
        get() {
          ...
          if (relatedFieldExists.value === true) {
            return (
              state.updateFields.find((updateField) => updateField.field === state.relations[0].field)?.meta
            ...
            return (
              state.newFields.find((newField) => newField.$type === 'manyRelated')?.meta?.interface ===
              'many-to-one'
            );
          }
        },
        set(enabled) {
          ...
          if (relatedFieldExists.value === true) {
            ...
            const newFieldCreated = !! state.newFields.find((newField) => newField.$type === 'manyRelated');
