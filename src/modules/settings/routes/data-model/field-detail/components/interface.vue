...
export default defineComponent({
  ...
  setup() {
    ...
    const selectItems = computed(() => {
      const type = state.fieldData?.type || 'alias';

      const recommendedInterfacesPerType = {
      ...
      const interfaceItems = availableInterfaces.value.map((inter) => {
        const item = {
          text: inter.name,
        ...
        if (recommended.includes(item.value)) {
          item.iconRight = 'star';
        }
      ...
      const interfaceList = interfaceItems.filter((item) => recommended.includes(item.value) === false);
