    ...
    function useOnCreate() {
      ...
      const onCreateOptions = computed(() => {
        if (state.fieldData.type === 'uuid') {
          ...
          if (props.type === 'm2o' && state.relations[0]?.related_collection === 'users') {
        ...
        set(newOption) {
          state.fieldData.meta = {
            ...
            special: (state.fieldData.meta?.special || []).filter(
              (special) => onCreateSpecials.includes(special) === false,
            ),
          };
      ...
      const onUpdateValue = computed({
        ...
        set(newOption) {
          state.fieldData.meta = {
            ...(state.fieldData.meta || {}),
            special: (state.fieldData.meta?.special || []).filter(
              (special) => onUpdateSpecials.includes(special) === false,
            ),
          };
