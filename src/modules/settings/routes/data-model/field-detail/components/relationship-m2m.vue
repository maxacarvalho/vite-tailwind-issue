    ...
    <div class="grid">
            ...
            <v-menu show-arrow placement="bottom-end">
              ...
              <v-list class="monospace">
                <v-list-item
                  ...
                  clickable
                  @click="junctionCollection = availableCollection.collection"
                >
                  <v-list-item-content>
                ...
                <v-list-group>
                  ...
                  <v-list-item
                    ...
                    clickable
                    @click="junctionCollection = systemCollection.collection"
                  >
                    <v-list-item-content>
      ...
      <div class="field">
        ...
        >
          <template #append>
            <v-menu show-arrow placement="bottom-end">
              <template v-if="!isExisting" #activator="{ toggle }">
                <v-icon
                  v-tooltip="t('select_existing')"
                  name="list_alt"
                  clickable
...
export default defineComponent({
  ...
  setup() {
    ...
    const availableCollections = computed(() => {
      return orderBy(
        collectionsStore.collections.filter((collection) => {
          return collection.collection.startsWith('system_') === false;
        }),
        ['collection'],
        ['asc'],
      );
    ...
    const systemCollections = computed(() => {
      return orderBy(
        collectionsStore.collections.filter((collection) => {
          return collection.collection.startsWith('system_') === true;
        }),
        ['collection'],
        ['asc'],
    ...
    const junctionFields = computed(() => {
      ...
      return fieldsStore.getFieldsForCollectionAlphabetical(junctionCollection.value).map((field) => ({
        text: field.field,
        value: field.field,
        disabled:
    ...
    function junctionFieldExists(fieldKey) {
    ...
    function useCorresponding() {
      const hasCorresponding = computed({
        get() {
          return !! state.newFields.find((field) => field.$type === 'corresponding');
        },
        set(enabled) {
          if (enabled === true && !! state.newFields.find((field) => field.$type === 'corresponding') === false) {
            state.newFields = [
              ...state.newFields,
              {
                $type: 'corresponding',
            ...
            state.newFields = state.newFields.filter((field) => field.$type !== 'corresponding');
            state.relations[1].meta = {
              ...(state.relations[1].meta || {}),
              one_field: null,
            };
      ...
      const correspondingField = computed({
        get() {
          return state.newFields?.find((field) => field.$type === 'corresponding')?.field || null;
        },
        set(field) {
          state.newFields = state.newFields.map((newField) => {
