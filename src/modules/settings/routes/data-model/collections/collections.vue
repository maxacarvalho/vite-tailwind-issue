    ...
    <div class="padding-box">
      ...
      >
        <template #[`item.icon`]="{ item }">
          <v-icon
            ...
            :class="{
							...
							system: item.collection.startsWith('system_'),
							unmanaged: item.meta === null && item.collection.startsWith('system_') === false,
						}"
            :name="item.icon"
            :color="item.color"
          />
        ...
        <template #item-append="{ item }">
          <v-icon
            v-if="!item.meta && item.collection.startsWith('system_') === false"
            v-tooltip="t('db_only_click_to_configure')"
            small
            class="no-meta"
            name="report_problem"
          />
          <collection-options v-if="item.collection.startsWith('system_') === false" :collection="item" />
        </template>
      </v-table>
    </div>
  ...
  setup() {
    ...
    function useItems() {
      const visible = computed(() => {
        return sortBy(
          collectionsStore.collections.filter(
            (collection) => collection.collection.startsWith('system_') === false && collection.meta?.hidden === false
          ),
          'collection'
        );
      });
      ...
      const hidden = computed(() => {
        return sortBy(
          collectionsStore.collections
            .filter(
              (collection) =>
                collection.collection.startsWith('system_') === false && collection.meta?.hidden === true
            )
      ...
      const system = computed(() => {
        return sortBy(
          collectionsStore.collections
            .filter((collection) => collection.collection.startsWith('system_') === true)
      ...
      const unmanaged = computed(() => {
        return sortBy(
          collectionsStore.collections
            .filter((collection) => collection.collection.startsWith('system_') === false)
