  ...
  setup(props, { emit }) {
    ...
    function useRelations() {
      ...
      const languagesRelation = computed(() => {
        if (! relationsForField.value) return null;
        return relationsForField.value.find((relation) => relation !== translationsRelation.value) || null;
      });
    ...
    function useLanguages() {
      ...
      async function fetchLanguages() {
        ...
        try {
          const response = await api.get(`/items/${languagesCollection.value}`, { params: { fields, limit: 999999 } });
            ...
            const existingEdit =
              props.value && Array.isArray(props.value)
                ? (props.value.find(
                  (edit) =>
                    isPlainObject(edit) &&
                    (edit)[languagesRelation.value.field] ===
