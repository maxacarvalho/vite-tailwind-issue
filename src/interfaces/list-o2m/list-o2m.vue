...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function getNewItems() {
      ...
      return props.value.filter((item) => typeof item === 'object' && pkField in item === false);
    }

    function getUpdatedItems() {
      const pkField = relatedPrimaryKeyField.value.field;
      if (props.value === null) return [];
      return props.value.filter((item) => typeof item === 'object' && pkField in item === true);
    }
    ...
    function usePreview() {
      ...
      watch(
        ...
        async () => {
          ...
          try {
            const endpoint = relatedCollection.value.collection.startsWith('system_')
              ? `/${relatedCollection.value.collection.substring(7)}`
