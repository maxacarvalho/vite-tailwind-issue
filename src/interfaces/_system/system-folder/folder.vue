<template>
  ...
  >
    <template #activator="{ toggle, active }">
      ...
      >
        <template #prepend>
          <v-icon :name="!value ? 'folder_special' : 'folder_open'" />
        </template>
        <template #append>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function folderParentPath(folder, folders) {
      ...
      const folderParent = (target) =>
        (folderMap.has(target.parent) ? folderParent(folderMap.get(target.parent)) : []).concat(target);
