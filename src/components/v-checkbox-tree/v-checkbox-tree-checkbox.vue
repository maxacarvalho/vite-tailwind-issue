...
export default defineComponent({
  ...
  setup(props, { emit }) {
    const visibleChildrenValues = computed(() => {
      ...
      function childrenHaveMatch(children) {
    ...
    function emitAll(rawValue, { added, removed }) {
    ...
    function emitBranch(rawValue, { added, removed }) {
      ...
      // When a child group is modified
      if (
    ...
    function emitLeaf(rawValue, { added }) {
      const allChildrenRecursive = getRecursiveChildrenValues('all');
      const leafChildrenRecursive = getRecursiveChildrenValues('leaf');
    ...
    function emitIndeterminate(rawValue, { added, removed }) {
      const childrenValuesRecursive = getRecursiveChildrenValues('all');
    ...
    function emitExclusive(rawValue, { added }) {
      const childrenValuesRecursive = getRecursiveChildrenValues('all');
    ...
    function getRecursiveChildrenValues(
      ...
      function getChildrenValuesRecursive(children) {
        if (! children) return;
