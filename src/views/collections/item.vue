    ...
    async function saveAndQuit() {
      ...
      try {
        ...
        if (props.singleton === false) {
          router.push(router.currentRoute.value.fullPath.replace(`/${props.primaryKey}`, ''));
        }
      } catch {
    ...
    async function saveAndStay() {
      ...
      try {
        ...
        if (props.primaryKey === '+') {
          ...
          router.replace(router.currentRoute.value.fullPath.replace('/+', `/${encodeURIComponent(newPrimaryKey)}`));
