...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const emails = ref('');
    const roles = ref([]);
    const roleSelected = ref(props.role);
    ...
    async function inviteUsers() {
      ...
      try {
        ...
        uniqueValidationErrors.value = err?.response?.data?.errors?.filter((error) => {
          return error.extensions?.code === 'RECORD_NOT_UNIQUE';
        });

        const otherErrors = err?.response?.data?.errors?.filter(
          (err) => err?.extensions?.code !== 'RECORD_NOT_UNIQUE',
        );

        if (otherErrors.length > 0) {
          otherErrors.forEach((err) => unexpectedError(err));
        }
      } finally {
        loading.value = false;
      }
