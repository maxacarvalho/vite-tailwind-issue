...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const showDivider = computed(() => {
      return (
        fieldsStore.getFieldsForCollection(props.collection).filter((field) => field.meta?.hidden !== true)
          .length > 0
      );
    });
    ...
    function useFile() {
      const isDirectusFiles = computed(() => {
        return junctionRelatedCollection.value === 'system_files';
      });
    ...
    function useItem() {
      ...
      async function fetchItem() {
        ...
        const endpoint = props.collection.startsWith('system_')
          ? `/${props.collection.substring(7)}/${props.primaryKey}`
      ...
      async function fetchRelatedItem() {
        ...
        const endpoint = collection.startsWith('system_')
          ? `/${collection.substring(7)}/${props.relatedPrimaryKey}`
    ...
    function useJunction() {
      ...
      function setJunctionEdits(edits) {
